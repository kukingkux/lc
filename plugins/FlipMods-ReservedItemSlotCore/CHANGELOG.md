# 2.0.55
+ Fixed issues on V80 where the new (vanilla) utility slot item would not sync with other players.
+ Fixed issue with the flashbang items not properly enabling mesh when equipped, and if the item is configured to appear on player while holstered.

# 2.0.54
+ Internal changes: Set the PlayerPatcher class to public.
