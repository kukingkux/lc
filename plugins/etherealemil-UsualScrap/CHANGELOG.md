...
- Fixed candy dispenser not spawning candy.
- Fixed tickets having incorrect references.
- Fixed not being able to load the vintage candy without the candy dispenser.
...
- Reverted whatever I did to the defib last update, hard to test the new display issue myself but this maybe fixes it.
...
- Fixed bandages randomly breaking.
- Fixed issues and exploits with the medical kit.
- Messed with defib tip displaying.
...
Pretty large code rewrite, thus it'll probably require a config refresh and a lobby wipe, sorry for the hassle. Possibly be prepared if an issue or two crops up, hopefully not.

- Modded moon spawn weights are now supported for all scrap items and are generated in this mods configs upon loading into a lobby for the first time. Format errors and moons that are installed or uninstalled are added and removed from the config automatically upon loading into a lobby. If you download 100 moons and load in for the first time, prepare for an absolute wall of warnings. Warnings are just the mod replacing incorrect formatting/spelling and adding or removing config entries for moons, they are just in case and are nothing to worry about.

-  Changed how to fuel cylinder works a fair bit. When dropped, its timer is still lowered by a percent but the amount of times you can drop it is now based on the time remaining and not a static 3 times. If dropped when the timer is at or below 20 seconds it will explode. Following that, reactivating the cylinder is now a 1/4 chance every drop instead of a static 3 drops. And finally, it will now deactivate when placed onto the company desk.

- Fuel cylinder is no longer conductive, taking into account the above changes and it's functions in general this was overkill.

- Normal chess pieces are no longer conductive, must've done that by mistake.

- Radioactive cell now plays an additional burn sound when damaging you.

- Piece of candy renamed to Vintage Candy, the "of" was annoying me a great deal. Vintage candies can now spawn on it's own very rarely and can be enable without the candy dispenser. If the candy dispenser is enabled then vintage candies are automatically loaded and enabled regardless of the config.

- Added metal sheets to the toolkits lootpool, this will get a rewrite soon to group low and high rarity loot options.

- flagged some leftover logging.

- For those config connoisseurs who desire control over every item's spawn rates, moon spawn configs are now available for individual scrap pack items. They are separated from the other items, disabled by default, and only take effect if the corresponding scrap pack is enabled.

- Until I can find a more consistent way of migrating over previous configs before resetting them, your configs will be moved to a backup before being overwritten.

- Another try at the defib revive icons updating, this is pretty tough to test out myself even with two accounts so for testing we'll just do it live 'til it works.


With another big rewrite aside, Toolkit additions and new items are actually up next.

Also we'll more than likely get to 1m downloads within the next few days probably before I can get the next serious update finalized, and with my birthday comming up in the next few days and the plans I have for the next update it may be a little bit so happy 1m! Thanks for everything.

As always, if you have any issues shoot me a report on the github or the lethal modding discord and I'll look into it ASAP, goooooooooooood night.

https://ko-fi.com/boxoemilios 
...
- Removed old tooltip line of code from the bandage.
- Fixed wrong scan node settings that worked for the longest time for some reason.
- Another try at the spectate boxes, seemed to work when I tested briefly.

Update 2.0.0 and we're very close to 1 million downloads, thank you all for trying and perhaps enjoying the mod everyone! even with all the random bugs its had.

Update ends here, below is just discussion.

---

There will be an update for 1 millions downloads, it just may not be out immediately after the milestone. I wanted it to line up as the 2.0 update but not everything works out perfectly. AAAaaanyway I'd thought of potentially working with the community to add a couple items people may have wanted because while I may have a seriously long list of potential items, the majority of them don't have a good niche or purpose in my opinion. I hadn't wanted to seem like I'm trying to steal ideas or anything like that so I had passed on doing this in the past but if you have an item idea and don't mind me adding it to this mod for this sort of celebration update, let me know on my github threads or the mod's post on the lethal company modding discord. 
...
<summary>Version 1.9.9</summary>

Hey! back from my trip so here's a couple fixes I could cook up asap, more to come possibly this weekend as usual if I have the time.

- When the defib is set to refill on landing its display will now correctly update.
- Fixed the previously updating tips only being for the host and always for the host. Let me know if this is still acting funny!
- Toolkit icon missing
- May have made changes before my trip but I cannot remember and didn't record them if so.


 Things I'm still figuring out/recoding - 
 - Defib perma death situations
 - Crowbar functions acts wonky on greenhouse doors
 
 Things I haven't finished yet - 
 - Updating spectate boxes
 - Modded moon spawn weights
 - Toolkit animations

Trip was out of my home country and I use a desktop so sorry for the wait!
...
<summary>Version 1.9.8</summary>

- Rewrote the Fuel Canister code, that thing wasn't workin' right.
- Added a donation link to the mod page, no need to donate as I will continue working on this as I have been but it is appreciated.
